(function(window, document) {
  ...
  var ListGroupCtrl = [
    ...
    function($scope, $attrs, $parse, $filter, $sce, $compile, $timeout) {
      ...
      ctrl.beforeSelectionChange = !$attrs.beforeSelectionChange
        ? defaultBeforeSelectionChange
        : $scope.beforeSelectionChange;
      ctrl.afterSelectionChange = !$attrs.afterSelectionChange
        ? angular.noop
      ...
      ctrl.executeFilter = function() {
        ctrl.$$items = $filter('filter')(
          $scope.items,
          ctrl.filter.text,
          ctrl.filter.comparator
        );
      };
      ...
      $scope.compare = function(actual, expected) {
        ...
        if (ctrl.filter.auto === true) {
          match = $filter(ctrl.filter.comparator)(
            actual,
            expected,
  ...
  var ListGroupEditorCtrl = [
    ...
    function(
      ...
      var title = $attrs.title
        ? $parse($attrs.title)($scope.$parent)
        : '&nbsp;';
  ...
  angular.module('listGroup').run([
    ...
    function($templateCache) {
      ...
      $templateCache.put(
        'panel-editable-list-group.tpl.html',
      ...
      $templateCache.put(
        'panel-list-group-title.tpl.html',
        '<h3 class="panel-title" ng-bind="::title"></h3>'
      );

      $templateCache.put(
        'panel-list-group.tpl.html',
